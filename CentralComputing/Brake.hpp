...
class Brake {
  public:
    ...
    /*
     * Uses a PI loop to calculate the appropriate Brake throttle
     */
    int16_t calculate_brakepower(double dt, int16_t last_throttle);

    /*
     * Sets the Brake throttle to a specific value
     */
    void set_brakepower(int16_t value);

    void debug_set_pwm_pin(std::string & path);

  private:
    int16_t Brake::calculate_throttle(double dt, int16_t last_throttle);
    void Brake::set_throttle(int16_t value);
